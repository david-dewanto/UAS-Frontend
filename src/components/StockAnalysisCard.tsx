...
export function StockAnalysisCard({ stockCode }: StockAnalysisCardProps) {
  ...
  const stockCodes = useMemo(() => (stockCode ? [stockCode] : []), [stockCode]);

  const { isLoading, error, getTrendStrength, getForecastMetrics } =
          ...
          <div className="space-y-6">
            ...
            <div className="text-sm text-muted-foreground">
              ...
              <ul className="list-disc pl-4 space-y-1">
                <li>
                  Trend strength indicates the reliability of the current price
                  movement
                </li>
                <li>
                  MAE (Mean Absolute Error) shows the average forecast deviation
                </li>
                <li>
                  RMSE (Root Mean Square Error) emphasizes larger forecast
                  errors
                </li>
              </ul>
            </div>
            <Alert className="mb-0 bg-gray-100 border-gray-200">
              <AlertDescription className="text-black-700 text-center">
                In Collaboration with Pintar Ekspor App (Arvyno Pranata Limahardja - 18222007)
              </AlertDescription>
            </Alert>
          </div>
        )}
